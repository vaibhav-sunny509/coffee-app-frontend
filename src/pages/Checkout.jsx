...
import { FaCreditCard, FaWallet, FaUniversity, FaMobileAlt, FaCcPaypal, FaMoneyBillAlt } from 'react-icons/fa';
...
const Checkout = ({ onPlaceOrder, cart }) => {
  ...
  const applyCoupon = (e) => {
    ...
    } else if (couponCode.toUpperCase() === "COFFEE10") {
        setDiscount(5); 
        setCouponMessage("Success! $5.00 Discount Applied.");
    } else {
        setDiscount(0);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onPlaceOrder(grandTotal.toFixed(2));
    ...
    if (paymentMethod === 'cod') {
        ...
        alert("Card Saved! Redirecting to Bank Security Page...");
      ...
      <div className="checkout-layout" style={{ display: 'flex', gap: '30px', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'stretch' }}>
        ...
        <form className="checkout-form" onSubmit={handleSubmit} style={{flex: 1, minWidth: '300px', margin: 0, display: 'flex', flexDirection: 'column'}}>
            ...
            <div className="form-group">
                ...
                <input type="text" placeholder="Apt 4B" required />
            </div>

            <div className="form-group">
                <label>Street Address</label>
                <input type="text" placeholder="123 Coffee Lane" required />
            </div>

            <div className="form-group">
                <label>City & Zip Code</label>
                <input type="text" placeholder="New York, 10001" required />
            </div>
            ...
            {/* Pushes button to the bottom if form stretches */}
            <div style={{marginTop: 'auto', paddingTop: '30px'}}>
                <button type="submit" className="add-btn" style={{ fontSize: '1.2rem', margin: 0 }}>
                    {paymentMethod === 'cod' ? `Confirm Order ($${grandTotal.toFixed(2)})` : (paymentMethod === 'card' ? `Save & Pay $${grandTotal.toFixed(2)}` : `Pay $${grandTotal.toFixed(2)}`)}
                </button>
            </div>
        </form>

        {/* Right Side: Order Summary & Coupon */}
        {/* 3. ADDED display: flex to wrapper, height: 100% to container */}
        <div style={{ flex: 0.8, minWidth: '300px', display: 'flex' }}>
            <div className="cart-container" style={{padding: '30px', width: '100%', height: '100%', display: 'flex', flexDirection: 'column'}}>
                ...
                <div style={{flex: 1}}> {/* Allows items to take up remaining space */}
                    {cart.map(item => (
                    ...
                    <div style={{marginBottom: '20px'}}>
                        ...
                        <div style={{display: 'flex', gap: '10px'}}>
                            <input type="text" placeholder="Enter Code (e.g. WELCOME2026)" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} style={{padding: '10px', flex: 1, border: '1px solid #ddd', borderRadius: '5px'}} />
                        ...
                        {couponMessage && <p style={{fontSize: '0.9rem', marginTop: '5px', color: discount > 0 ? '#27ae60' : '#e74c3c'}}>{couponMessage}</p>}
                    </div>
                ...
                {/* 4. ADDED marginTop: auto to push Total to the bottom */}
